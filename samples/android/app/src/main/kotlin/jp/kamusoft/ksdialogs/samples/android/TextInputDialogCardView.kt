 ...
 * @param onComplete 完了操作。入力中の文字列を結果として渡す
 */
internal class TextInputDialogCardView(
    context: Context,
    message: String,
    onCancel: () -> Unit,
    ...
    init {
        ...
        addView(
            ...
            LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT).apply {
                topMargin = context.dp(ROW_SPACING_DP)
            },
        )
        addView(
            buttonRow(context, onCancel) { onComplete(inputField.text.toString()) },
            LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT).apply {
                topMargin = context.dp(ROW_SPACING_DP)
            },
        )
    ...
    /** 文字列を入れる欄を作る。初期値は空で、下書き文言だけが見えている。 */
    private fun inputField(context: Context): EditText = EditText(context).apply {
        hint = SampleText.TEXT_INPUT_PLACEHOLDER
        inputType = InputType.TYPE_CLASS_TEXT
        gravity = Gravity.START or Gravity.CENTER_VERTICAL
        setTextSize(TypedValue.COMPLEX_UNIT_SP, FIELD_TEXT_SIZE_SP)
        setTextColor(SampleTheme.ON_SURFACE)
        setHintTextColor(SampleTheme.ON_SURFACE_MUTED)
        ...
        // 入力欄も Material の推奨タップ領域 48dp を下限にする
        minHeight = context.dp(FIELD_MIN_HEIGHT_DP)
        minimumHeight = context.dp(FIELD_MIN_HEIGHT_DP)
    }
    ...
    private companion object {
        ...
        const val FIELD_TEXT_SIZE_SP = 15f
        const val FIELD_CORNER_RADIUS_DP = 10
        const val FIELD_STROKE_WIDTH_DP = 1
        const val FIELD_HORIZONTAL_PADDING_DP = 12
        const val FIELD_MIN_HEIGHT_DP = 48
