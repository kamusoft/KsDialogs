...
/// 載せた View は四辺で固定する。
final class TransitionDialogCardHostView: UIView {
    private let hostingController: UIHostingController<TransitionDialogCard>

    init(message: String, onCancel: @escaping () -> Void, onComplete: @escaping () -> Void) {
        hostingController = UIHostingController(
            rootView: TransitionDialogCard(message: message, onCancel: onCancel, onComplete: onComplete)
        )
