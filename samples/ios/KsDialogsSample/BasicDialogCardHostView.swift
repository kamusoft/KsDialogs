...
/// 載せた View は四辺で固定する。
final class BasicDialogCardHostView: UIView {
    private let hostingController: UIHostingController<BasicDialogCard>

    init(message: String, onCancel: @escaping () -> Void, onComplete: @escaping () -> Void) {
        hostingController = UIHostingController(
            rootView: BasicDialogCard(message: message, onCancel: onCancel, onComplete: onComplete)
        )
