...
struct SampleTransitionPanelScreen: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 0) {
                    SampleSectionCaption(title: SampleText.shared.PRESET_CAPTION)
                    presetChips
                    SampleSectionCaption(title: SampleText.shared.ADJUST_CAPTION)
                    adjustBlock
    ...
    private var adjustBlock: some View {
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                Text(SampleText.shared.DURATION_LABEL)
                    .font(.system(size: 14))
            ...
            Slider(value: $model.durationMilliseconds, in: 100...600, step: 10)
                ...
                .accessibilityLabel(SampleText.shared.DURATION_LABEL)
                .accessibilityValue(model.durationText)

            Text(SampleText.shared.EASING_LABEL)
                .font(.system(size: 14))
                .foregroundStyle(SampleTheme.onSurface)
    ...
    private var showButton: some View {
        Button {
            ...
            Text(SampleText.shared.DISPLAY_ACTION)
