...
final class SampleTransitionPanelModel {
    ...
    var durationText: String {
        SampleText.shared.durationValue(milliseconds: Int32(durationMilliseconds.rounded()))
    }
    ...
    func showTransitionDialog() async -> String? {
        do {
            // 演出は show の引数では渡せないため、選択を共有 Presenter 越しに View 定義側へ運ぶ
            lastResult = try await presenter.showTransitionDialog(
                preset: transitionChoice.preset,
                durationMilliseconds: Int32(durationMilliseconds.rounded()),
                easing: easingChoice.preset
            )
            return lastResult
        } catch {
