 ...
 * 報告口を factory の引数で受け取らず、表示中の共有 ViewModel から `notifier` で引く — だけである。
 *
 * @param viewModel 共有コードで定義された、表示中の ViewModel
 */
internal class ModelDialogCardView(
    context: Context,
    viewModel: ModelDialogViewModel,
    ...
    init {
        ...
        addView(messageLabel(context, viewModel.message))
        addView(
            buttonRow(
                context = context,
                onCancel = { viewModel.notifier?.cancel() },
                onComplete = { viewModel.notifier?.complete(true) },
            ),
    ...
    private companion object {
        ...
        const val MESSAGE_BOTTOM_SPACING_DP = 20
