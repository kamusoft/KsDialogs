...
#                   Maven Central への発行は Android 枠 (ksdialogs-core / ksdialogs) と
#                   KMP 枠 (ksdialogs-kmp の全 publication) の 2 つの deployment に分かれる
#                   ため、判定に使う artifactId を第 2 引数で選べる。既定は Android 枠の
#                   代表 `ksdialogs-core` で、KMP 枠は `ksdialogs-kmp` を渡す。枠ごとに
#                   代表を分けないと、片方だけ公開済みの再実行で未公開の枠を skip してしまう。
#
...
# published の既定の判定対象 (Android 枠の代表)。
readonly DEFAULT_MAVEN_ARTIFACT_ID="ksdialogs-core"
...
maven_pom_url() {
    local version="$1"
    local artifact_id="${2:-${DEFAULT_MAVEN_ARTIFACT_ID}}"
...
cmd_published() {
    ...
    local artifact_id="${2:-${DEFAULT_MAVEN_ARTIFACT_ID}}"
    local url status
    url="$(maven_pom_url "${version}" "${artifact_id}")"
    status="$(response_status "$(http_request HEAD "${url}" noauth)")"
    case "${status}" in
        200)
            echo "公開済み: ${artifact_id}:${version}"
            return 0
            ;;
        404)
            echo "未公開: ${artifact_id}:${version}"
            return 1
            ;;
        *)
...
selftest() {
    ...
    check "$([ "$(maven_pom_url 1.2.3-beta.4)" = "https://repo1.maven.org/maven2/jp/kamusoft/ksdialogs-core/1.2.3-beta.4/ksdialogs-core-1.2.3-beta.4.pom" ] && echo 0 || echo 1)" \
        "Maven Central の pom の URL (既定は Android 枠)" "$(maven_pom_url 1.2.3-beta.4)"
    check "$([ "$(maven_pom_url 1.2.3-beta.4 ksdialogs-kmp)" = "https://repo1.maven.org/maven2/jp/kamusoft/ksdialogs-kmp/1.2.3-beta.4/ksdialogs-kmp-1.2.3-beta.4.pom" ] && echo 0 || echo 1)" \
        "Maven Central の pom の URL (KMP 枠)" "$(maven_pom_url 1.2.3-beta.4 ksdialogs-kmp)"
    ...
    check "$([ "$(calls)" = "HEAD https://repo1.maven.org/maven2/jp/kamusoft/ksdialogs-core/1.2.3/ksdialogs-core-1.2.3.pom noauth" ] && echo 0 || echo 1)" \
        "published は認証なしの HEAD 1 件 (既定は Android 枠)" "$(calls)"

    arrange '200 '
    published_code=0
    ( cmd_published 1.2.3 ksdialogs-kmp > /dev/null 2>&1 ) || published_code=$?
    check "$([ "${published_code}" = "0" ] && echo 0 || echo 1)" "KMP 枠も 200 なら公開済み" "exit ${published_code}"
    check "$([ "$(calls)" = "HEAD https://repo1.maven.org/maven2/jp/kamusoft/ksdialogs-kmp/1.2.3/ksdialogs-kmp-1.2.3.pom noauth" ] && echo 0 || echo 1)" \
        "artifactId 引数が判定対象の座標を切り替える" "$(calls)"
...
main() {
    ...
    case "${subcommand}" in
        --selftest)
            ...
            ;;
        published)
            # 判定に使う artifactId は省略できる (既定は Android 枠の代表)。
            if [ $# -lt 1 ] || [ $# -gt 2 ]; then
                usage
                exit 2
            fi
