...
検査する内容:
  ...
  - 引き継ぎを取ってくる step (引き継ぎの置き場へ download する step) が 1 件ある
  - 引き継ぎを読む step (読み込み結果を出力する step) が 1 件ある
...
続行可否の印の download は「他の成果物」に数えない。印は引き継ぎを読むかどうかを決める判定
そのものの入力で、publish が配布のために取ってくる成果物ではない。引き継ぎの download は
その判定の結果を条件に持つため、構造として印より前へは動かせない。
...
# 引き継ぎの置き場。download する step をこの path の末尾で見分ける。
HANDOVER_PATH_MARKER = "/deployment"

# 引き継ぎを読むかどうかの判定に使う成果物の置き場。publish の成果物ではないので、
# 「他の成果物の取得」からは除く。
DECISION_PATH_MARKERS = ("/eligibility",)

# 引き継ぎを読む step を見分ける印。読み込み結果を後続の step へ渡す id で見る。
HANDOVER_INSPECT_MARKER = "id: deployment-ids"
...
def has_path_marker(body: str, marker: str) -> bool:
    """step の `path:` が指定の marker で終わっているかを返す。"""
    return bool(re.search(r"^\s+path:.*" + re.escape(marker) + r"\s*$", body, re.MULTILINE))


def classify(steps: list[tuple[str, str]]) -> tuple[list[int], list[int], list[int]]:
    """step を (引き継ぎの download / 引き継ぎの読み込み / 他の成果物の download) に分ける。"""
    ...
    for index, (_, body) in enumerate(steps):
        ...
        if DOWNLOAD_ACTION_MARKER not in body:
            continue
        if has_path_marker(body, HANDOVER_PATH_MARKER):
            handover_download.append(index)
        elif any(has_path_marker(body, marker) for marker in DECISION_PATH_MARKERS):
            continue
        else:
            other_downloads.append(index)
    return handover_download, handover_inspect, other_downloads
...
def check(release_yml: str) -> int:
    ...
    handover_download, handover_inspect, other_downloads = classify(steps)
...
def step_bounds(lines: list[str], name: str) -> tuple[int, int]:
    ...
    """
    header = f"      - name: {name}"
    assert header in lines, f"自己テストの土台が変わっている: step '{name}' が見つからない"
...
def delete_other_downloads(text: str) -> str:
    """他の成果物を download する step を全て消した workflow を返す。

    件数は数えずに分類の結果から名前を引く。download する成果物が増えても自己テストの
    土台が崩れないようにするため。
    """
    steps = publish_steps(text)
    _, _, other_downloads = classify(steps)
    assert other_downloads, "自己テストの土台が変わっている: 他の成果物の download が無い"
    for index in other_downloads:
        text = delete_step(text, steps[index][0])
    return text


def replace_once(text: str, old: str, new: str) -> str:
    """印を別の文字列へ置き換えた workflow を返す (自己テストの入力を作る)。"""
    assert old in text, f"自己テストの土台が変わっている: '{old}' が見つからない"
...
def selftest() -> int:
    ...
    handover_download = "Download previous deployment ids"
    handover_inspect = "Prepare deployment id files"
    after_downloads = "Configure distribution repository access"
    ...
    cases: list[tuple[str, str, int]] = [
        ...
        (
            ...
            move_step(original, handover_inspect, after_downloads),
            1,
        ),
        (
            "引き継ぎの download を他の成果物の取得より後ろへ動かすと落ちる",
            move_step(original, handover_download, after_downloads),
            1,
        ),
        ("引き継ぎの download を消すと落ちる", delete_step(original, handover_download), 1),
        (
            "引き継ぎの読み込みを消すと落ちる",
            replace_once(original, HANDOVER_INSPECT_MARKER, "id: handover-removed"),
            1,
        ),
        ("他の成果物の取得が無くなると落ちる", delete_other_downloads(original), 1),
        (
            "続行可否の印の download は他の成果物に数えない",
            replace_once(
                original,
                f"path: ${{{{ runner.temp }}}}{DECISION_PATH_MARKERS[0]}",
                "path: ${{ runner.temp }}/decision-marker",
            ),
            1,
        ),
    ]
