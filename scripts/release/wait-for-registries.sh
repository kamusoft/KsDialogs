...
#
# 照会の結果は対象ごとに 4 つへ分類する。判定不能はさらに失敗の種別まで残す。
#
...
# Maven Central の当該座標・当該 version の pom を照会して分類する。
#
...
maven_probe() {
    ...
    response="$(http_get HEAD "$(maven_pom_url "${artifact_id}" "${version}")")"
...
# --- 待機の対象 --------------------------------------------------------------------
#
# 10 対象 (Maven 座標 7 件と nuget.org の Package ID 3 件) を、並びの同じ 5 本の配列で
...
reset_targets() {
    ...
    for artifact_id in "${MAVEN_ARTIFACT_IDS[@]}"; do
        TARGET_KIND+=("maven")
        TARGET_KEY+=("${artifact_id}")
        TARGET_LABEL+=("Maven Central ${MAVEN_GROUP_PATH//\//.}:${artifact_id}")
    done
...
probe_target() {
    ...
    if [ "${TARGET_KIND[${index}]}" = "maven" ]; then
        maven_probe "${TARGET_KEY[${index}]}" "${version}"
    else
...
selftest() {
    ...
    # 同じ応答を n 行ぶん並べる補助。引数: <行数> <応答の行>
    repeat_lines() {
        local count="$1" line="$2" i
    ...
    all_calls_use_full_max_time() {
        if [ "$(grep -c "max-time=${HTTP_MAX_TIME_SECONDS}$" "${MOCK_CALLS}")" = "$(call_count)" ]; then
            echo 0
        else
            echo 1
        fi
    ...
    probe_once() {
        arrange "$1"
        if [ "$2" = "maven" ]; then
            maven_probe ksdialogs-core 1.2.3
        else
            nuget_probe ksdialogs.maui 1.2.3
        fi
    }
    ...
    reset_targets
    check "$([ "${#TARGET_KIND[@]}" = "10" ] && echo 0 || echo 1)" \
        "対象の配列も 10 件" "${#TARGET_KIND[@]}"
    check "$([ "${TARGET_STATE[0]}" = "${STATE_UNPROBED}" ] && [ "${TARGET_STATE[9]}" = "${STATE_UNPROBED}" ] && echo 0 || echo 1)" \
        "初期値は未照会" "${TARGET_STATE[0]} / ${TARGET_STATE[9]}"

    echo "[URL の組み立て]"
    check "$([ "$(maven_pom_url ksdialogs-kmp-iosarm64 1.2.3-beta.4)" = "https://repo1.maven.org/maven2/jp/kamusoft/ksdialogs-kmp-iosarm64/1.2.3-beta.4/ksdialogs-kmp-iosarm64-1.2.3-beta.4.pom" ] && echo 0 || echo 1)" \
        "Maven Central の pom の URL" "$(maven_pom_url ksdialogs-kmp-iosarm64 1.2.3-beta.4)"
    ...
    check "$([ "$(call_targets)" = "HEAD https://repo1.maven.org/maven2/jp/kamusoft/ksdialogs-core/1.2.3/ksdialogs-core-1.2.3.pom" ] && echo 0 || echo 1)" \
    ...
    check "$([ "$(call_targets)" = "GET https://api.nuget.org/v3-flatcontainer/ksdialogs.maui/index.json" ] && echo 0 || echo 1)" \
    ...
    outcome="$(nuget_probe ksdialogs.maui 1.2.3-beta.1)"
    ...
    outcome="$(probe_once '200 ["1.2.3"]' nuget)"
    check "$([ "${outcome%% *}" = "unknown-parse" ] && echo 0 || echo 1)" \
        "最上位が object でない応答は判定不能 (応答を解釈できない)" "${outcome}"
    ...
    # 巡回の検査。台本は Maven 7 件 → nuget 3 件の順に消費される。巡回を回す検査には
    # 必ず短い上限を与える — 既定 (45 分) のままだと、照会回数が増える回帰が「数秒で
    # NG」ではなく「呼び出し側のタイムアウト」として出る。台本切れの印もあわせて見る。
    echo "[巡回]"
    local output=""
    local code=0
    ...
    # 10 対象が 反映済み / 未反映 / 判定不能 3 種別 に分かれる台本にする。
    arrange \
        '200 ' \
        '404 ' \
        '200 ' \
    ...
    check "$([ "$(call_count)" = "10" ] && echo 0 || echo 1)" \
        "上限に達する実行でも 10 対象を 1 巡だけ照会する" "$(calls)"
    ...
    check "$(contains "${output}" "Maven Central jp.kamusoft:ksdialogs-core: 反映済み")" \
        "混在する巡回で反映済みの Maven 座標が残る" "${output}"
    check "$(contains "${output}" "Maven Central jp.kamusoft:ksdialogs: 未反映")" \
        "混在する巡回で未反映の Maven 座標が残る" "${output}"
    check "$(contains "${output}" "Maven Central jp.kamusoft:ksdialogs-kmp-iossimulatorarm64: 判定不能 (応答が成功を示さない: HTTP 503)")" \
        "混在する巡回で KMP ターゲットの判定不能が種別つきで残る" "${output}"
    check "$(contains "${output}" "Maven Central jp.kamusoft:ksdialogs-kmp-iosx64: 判定不能 (応答を解釈できない: ステータスコードを読み取れない)")" \
        "混在する巡回で解釈不能が種別つきで残る" "${output}"
    check "$(contains "${output}" "nuget.org/ksdialogs.maui: 反映済み")" \
        "混在する巡回で反映済みの Package ID が残る" "${output}"
    check "$(contains "${output}" "nuget.org/ksdialogs.binding.ios: 未反映")" \
        "混在する巡回で未反映の Package ID が残る" "${output}"
    check "$(contains "${output}" "nuget.org/ksdialogs.binding.android: 判定不能 (通信そのものの失敗)")" \
        "混在する巡回で通信の失敗が残る" "${output}"
    ...
    arrange \
        "$(repeat_lines 7 '200 ')" \
        "$(repeat_lines 3 '200 {"versions":["1.2.2"]}')" \
        "$(repeat_lines 3 '200 {"versions":["1.2.3"]}')"
    code=0
    ...
    check "$([ "$(call_count)" = "13" ] && echo 0 || echo 1)" \
    ...
    check "$([ "$(call_count)" = "11" ] && echo 0 || echo 1)" \
        "回復後は再照会しない (2 巡目は回復した 1 件だけ)" "$(calls)"
    ...
    POLL_DEADLINE=$(( SECONDS - 1 ))
    poll_once 1.2.3
    check "$([ "$(call_count)" = "0" ] && echo 0 || echo 1)" \
    ...
    # 期限までまだ余裕があれば、同じ入口から 10 対象すべてを照会する。
    POLL_DEADLINE=$(( SECONDS + 3600 ))
    poll_once 1.2.3
    check "$([ "$(call_count)" = "10" ] && echo 0 || echo 1)" \
        "期限内なら 10 対象すべてを照会する" "$(calls)"
    ...
    poll_once 1.2.3
    rm -f "${MOCK_DELAY}"
    check "$([ "$(call_count)" = "1" ] && echo 0 || echo 1)" \
    ...
    for unprobed_index in 1 4 7 9; do
    ...
    check "$(contains "${output}" "nuget.org/ksdialogs.maui: 未照会")" \
