...
# 行頭の空白も許さない — 箇条書きの入れ子に見える記載を黙って `- none` として扱わないため。
NONE_MARKER = "- none"
...
def selftest_parsing() -> list[tuple[str, bool]]:
    ...
    notes = render_notes(
        [
            {"number": 42, "body": changes_body(
                "- fix: 表示待ちの取りこぼしを直した",
                "- breaking: ダイアログの色指定が非 null になった",
            )},
            {"number": 45, "body": changes_body("- feature: Toast の表示位置を指定できる")},
        ],
        repo, "1.0.0", "0.9.0",
    )
    ...
    check("複数 pull request 分が連結される",
          "- Toast の表示位置を指定できる (#45)" in notes
          and "- 表示待ちの取りこぼしを直した (#42)" in notes)
    ...
    check("`- none` と項目の共存は失敗する",
          fails([{"number": 3, "body": changes_body("- none", "- fix: 直した")}],
                "単独でのみ書ける"))
    check("字下げした `- none` は失敗する",
          fails([{"number": 3, "body": changes_body("  - none")}], "認識できない行"))
...
def selftest_selection() -> list[tuple[str, bool]]:
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        head = commits["third"]
        # この一覧は古い順のまま置く (新しい順の newest_first と向きを違えることで、
        # 順位付けが一覧の端を採る退行を両方向とも落とす)。
        published = [
        ...
        # GitHub の Release 一覧は新しい順に返る。祖先が 2 件あり、起点がその並びの
        # 先頭でも末尾でもない状態を作る — 順位付けが「距離が最小」から「並びの端を
        # 採る」へ退行しても、一覧が短いと結果が変わらず素通りするため。
        newest_first = [
            {"tag_name": "0.9.9", "draft": False},
            {"tag_name": "0.1.0-beta.2", "draft": False},
            {"tag_name": "0.1.0-beta.1", "draft": False},
        ]
        results.append((
            "新しい順の一覧でも最も近い祖先が起点になる",
            select_base(newest_first, "0.1.0-beta.3", work, head)
            == ("0.1.0-beta.2", commits["second"]),
        ))
        results.append((
            "起点が無ければ履歴の最初から集める",
