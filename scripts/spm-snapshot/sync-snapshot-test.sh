...
readonly DISTRIBUTION_ORIGIN="https://github.com/kamusoft/KsDialogs-SPM.git"
...
destination="$(make_destination "wrong-origin" "https://github.com/kamusoft/KsDialogs.git")"
...
destination="$(make_destination "lookalike-origin" "https://github.com/evil/kamusoft/KsDialogs-SPM.git")"
touch "${destination}/keep.txt"
run_sync "${destination}"
assert_equals "1" "$([ $? -ne 0 ] && echo 1 || echo 0)" "非ゼロ終了する"
assert_equals "./keep.txt" "$(snapshot_tree "${destination}")" "同期先の内容が変更されない"
