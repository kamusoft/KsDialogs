...
Failures after acceptance — a factory failure, a failure to attach to the container, or a failure thrown by the view-model factory or `configure` of a `Show` that takes the view-model type — do not become exceptions. A warning is recorded, that single Toast is discarded, and nothing returns to the caller, which has already returned. When the view bound by `RegisterForToast` could not be constructed, `DialogException.ViewCreationFailed` is what the warning records as the cause, with the original failure in its `InnerException` and the pair of type names in `ViewTypeName` and `ViewModelTypeName` ([DI registration](di-registration.md)). Other Toasts are unaffected. Type-based presentation in Dialog and Loading propagates these failures to the caller instead, so Toast is the one that differs.
