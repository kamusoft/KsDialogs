...
/// 提示・通知の到着のような非同期の状態変化を待ち合わせるための補助。
enum BridgeTestWaiting {
    ...
    ///
    /// 到着そのもの (通知が届く・器が 1 枚増える) のように、一度成り立てば覆らない条件に使う。
    ...
    struct Outcome {
        ...
        private let history: BridgeTestStateHistory

        init(settled: Bool, history: BridgeTestStateHistory) {
            self.settled = settled
    ...
    static func awaitSettled(
        ...
        let history = BridgeTestStateHistory()
        let deadline = ContinuousClock.now.advanced(by: timeout)
...
@MainActor
final class BridgeTestStateHistory {
