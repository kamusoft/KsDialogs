...
/// </summary>
/// <remarks>
/// 結果報告口も進捗の受け口も持たないこと以外は <see cref="DialogViewFactory"/> と同じ役割で、
/// ViewModel を受け取り、その Toast の中身となる MAUI View を新規に生成する。
/// </remarks>
/// <param name="viewModel">表示に渡された ViewModel。登録キーと同じ型であることは解決側が保証する。</param>
/// <returns>Toast の中身になる MAUI View。</returns>
internal delegate View ToastViewFactory(object viewModel);
...
/// </remarks>
internal static class ToastViewFactories
{
    ...
    public static ToastViewFactory Erase<TViewModel>(Func<TViewModel, View> factory)
