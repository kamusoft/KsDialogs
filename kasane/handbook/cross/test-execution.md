...
`develop` への push と `main` 宛ての pull request で検証 CI が起動し、上表の実行を 5 つの job に分けて回す。CI が回す範囲・手元に残る範囲・CI だけの実行条件 (ios job のスイート直列化) は [検証 CI の範囲と実行条件](verification-ci.md) が持つ。変更の完了を判定するときは CI の緑だけでは足りず、同文書の「手元に残る」項目を手で回す。
