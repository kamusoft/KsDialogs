...
| [cross](cross/index.md) | リポジトリ横断の規約 (13件 — コメント・テスト実行・検証 CI の範囲と実行条件・実行時挙動の検証・Sample パリティ・移植元参照・ローカル開発環境の準備・利用者向け Skill の API 掲載基準・利用者向け Skill の記述スタイル・docs-refresh を走らせる時点・診断文言の言語・状態遷移の観測と CI 限定スキップ・CI で動くスクリプト内の削除) |
