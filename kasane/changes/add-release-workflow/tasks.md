...
- [x] 6.1 `main` を `develop` の先端から作成し、直後に branch protection を完全な payload で PUT する (必須 status check 10 件を `{"context": ..., "app_id": 15368}` 形式、PR 必須・承認数 0、force-push / 削除禁止、admin バイパス許容)。`develop` の保護は変えない。証跡 (`gh api` の応答) を evidence に残す (→ Requirement: main のマージ保護)
