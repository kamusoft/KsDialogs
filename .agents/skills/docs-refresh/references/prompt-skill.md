...
KsDialogs プロジェクトの利用者向け Agent Skill の追従更新を行ってください。
...
## 変更理由
{{差分検出で得られたもの。例: "core/api/loading-semantics.md が更新され、Loading の多重表示の合流契約が追記された"}}

## 内容規約 (必須)
① 構成は「能力マップ + レシピ形式 + 段階開示」に従う。
...
⑤ 閉世界性を守る。kasane/ 配下の内部文書・ADR 番号・利用者向けでない機械面
   (KsDialogsInteropBridge / KsDialogsInteropResultType — 現存する public ABI だが
    KMP cinterop 委譲専用で、利用者の入口は Swift パッケージ側の型付き面) を書かない。
   他の Skill やリポジトリ内の他ファイルへの相対リンクも張らない (リンクは自分の
   Skill ルート skills/{en,ja}/<name>/ の内側だけ。他の Skill には Skill 名で言及する)。
   外部への言及は配布座標 (SwiftPM の package URL・frontmatter の metadata.source) に限り、
   他リポジトリの解説へ読者を送る参照は書かない。
