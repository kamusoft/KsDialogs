...
import jp.kamusoft.ksdialogs.ToastPlacementDefault
import jp.kamusoft.ksdialogs.toastLayoutSnapshot
...
/**
 * Toast の器が組み立てた面を実際の画面に載せ、レイアウト完了後の外形を測る。
 *
 * rect の計算だけを確かめても測定条件や制約への反映漏れは見つからないため、実際のレイアウトパスを
 * 通した実測で確かめる (core/ADR-0009)。器が変われば反映漏れの形も変わるので、Dialog / Loading の
 * 器とは別に Toast の器でも同じ表を全量回す。
 *
 * ウィンドウの大きさとシステム領域の余白はケースが与えた値を使う (環境差を期待値の分岐ではなく
 * 入力の差で表すため)。器のウィンドウは端末の画面より大きくできないので、器が組み立てた面だけを
 * ケースの寸法で画面へ載せる — 面の組み立て (実効値の供給元の結び付け・測定条件) は
 * 本番と同じ [ToastContainer] が行う。
 */
internal object ToastLayoutMeasurement {
    ...
    fun measureToastRect(
        ...
        scenario.onActivity { activity ->
            ...
            val container = ToastContainer(
                ...
                layoutSnapshot = toastLayoutSnapshot(
                    contentView = contentView,
